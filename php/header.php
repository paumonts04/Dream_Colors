<?php
require_once 'conexion.php'; // El dir era para que pillara la ruta

$headerBaseUrl = '/Dream_Colors';

$headerClasses = ['site-header'];

if (($headerBanner ?? '') === 'cabello') { // Esto es para que cada banner sea diferente dependiendo de la categoria
    $headerClasses[] = 'site-header-cabello';
} elseif (($headerBanner ?? '') === 'capilar') {
    $headerClasses[] = 'site-header-capilar';
} elseif (($headerBanner ?? '') === 'facial') {
    $headerClasses[] = 'site-header-facial';
}
...
$sqlServicios = "
    ...
    FROM categoria
    JOIN servicios ON servicios.id_categoria = categoria.id
